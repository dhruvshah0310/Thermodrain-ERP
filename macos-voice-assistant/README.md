...
| `showOverlay` | On by default. Shows a Siri-style floating window with **Sumo** (Thermodrain's mascot) rendered as a 3D particle figure (Three.js/WebGL in a transparent WKWebView) while Jarvis is active. The status text below updates (Listening / Working on it / the reply). Needs a network connection to load Three.js. Toggle from the menu (**Show Siri-style Window**). |
